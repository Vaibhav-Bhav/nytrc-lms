...
export function AdminStudents({
  ...
  function SortTh({ field, label, cls }: { field: string; label: string; cls?: string }) {
    ...
    return (
      <th className={cn("text-left px-4 py-3 first:pl-5", cls)}>
        <button
    ...
    <AdminLayout current="admin-students" onNavigate={onNavigate}>
      <main className="flex-1 p-4 sm:p-8">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-5 sm:mb-8">
          <div>
            <h1 className="text-xl sm:text-2xl font-semibold text-foreground">Students</h1>
                    ...
                    >
                      <td className="px-5 py-3">
                        <div className="flex items-center gap-2.5">
                          ...
                          <span className="text-sm font-semibold text-foreground hover:text-primary transition-colors text-left whitespace-nowrap">
                            {student.name}
                          </span>
                        </div>
                      </td>
                      <td className="px-4 py-3 text-sm text-muted-foreground hidden md:table-cell">{student.email}</td>
                      <td className="px-4 py-3 text-sm text-muted-foreground whitespace-nowrap">{student.joined}</td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs font-semibold text-muted-foreground tabular-nums">{student.progress}%</span>
                        </div>
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3 text-right">
                        <div className="p-1.5 inline-flex items-center justify-center rounded hover:bg-muted transition-colors">
